...
document.addEventListener("DOMContentLoaded", () => {
        ...
        if (window.innerWidth <= 768) {
            ...
            links.forEach(elem => {
                elem.addEventListener("click", () => {
                    if (!elem.closest(".header_links_first_submenu")) {
                        menu.classList.toggle("-active");
                    }
    ...
    //    Слайдеры
    try {
        const otzyvy_slider = new Carousel(document.querySelector(".otzyvy_slider_js"));
        const portfolio_slider = new Carousel(document.querySelector(".portfolio_slider_js"), {
            Dots: false
        });
        const sotrudniki_slider = new Carousel(document.querySelector(".sotrudniki_slider_js"), {
            Dots: false
        });
    } catch (e) {
...
//     Логика для всех блоков данного типа
    ...
    function showMoreLogic(countVisible, classElements, classBtn, classParent) {
        let elements, btn, parent;
        let step = 1, parentHeight = 0;
        ...
        // скрою элменты (все кроме 6)
        elements.forEach((elem, i) => {
            if (i > 5) elements[i].classList.add("-hide");
        });

        /*for (let i = countVisible-1; i++; i < elements.length) {
            //
        }*/

        // скрою кнопку если элементов ровно countVisible
        if (elements.length === countVisible - 1) {
            btn.classList.add("-hide");
        } else btn.dataset.event = "go";


        // Обработчик клика на кнопку
        btn.addEventListener("click", () => {
            // Логика для скрытия списка
            if (btn.dataset.event === "hide") {
                step = 1;
                btn.dataset.event = "go";
                btn.textContent = "Показать все ↓";
                // чтобы экран пользователь оставался на том же месте где и был
                parentHeight = parent.getBoundingClientRect().height - (elements[0].getBoundingClientRect().height * 2);
            ...
            // Логика для показа элементов
            // Открою еще элементы
            for (let i = step * countVisible - 1; i < ((step + 1) * countVisible); i++) {
                if (i <= elements.length - 1) {
                    elements[i].classList.remove("-hide");
                } else {
                    btn.textContent = "Скрыть";
                    btn.dataset.event = "hide";
                    step = 1;
                    break
                }
            }
    ...
    myTabs("price_table_js", "price_table_btn_js", "price_table_content_js");
    ...
    * Copyright (c) 2019 Apoorv Saxena; Licensed MIT */
    ...
    function removeLink() {
        try {
            ...
            console.log("Нет ссылок для удаления")
        }
    }
    ...
    function filterElements() {
        ...
        button.addEventListener("click", () => {
            ...
            // ФИЛЬТР СПЕРВА ДЕШЕВЛЕ / ДОРОЖЕ
            if (mont_price.dataset.def !== "true") {
                if (mont_price.dataset.oborud === "deshevo") {
                    newListElements = newListElements.sort((a, b) => {
                        let price_a = parseInt(a.querySelector('.f_septik_price_js').dataset.price);
                        let price_b = parseInt(b.querySelector('.f_septik_price_js').dataset.price);
                        return price_a > price_b ? 1 : price_b > price_a ? -1 : 0;
                    })
                } else if (mont_price.dataset.oborud === "dorogo") {
                    newListElements = newListElements.sort((a, b) => {
                        let price_a = parseInt(a.querySelector('.f_septik_price_js').dataset.price);
                        let price_b = parseInt(b.querySelector('.f_septik_price_js').dataset.price);
                        return price_a > price_b ? -1 : price_b > price_a ? 1 : 0;
...
//    Убрать лишний текст из форм
    function addPoliticaText() {
        try {
            let tags = document.querySelectorAll(".politica_js");
            tags.forEach(elem => {
                elem.innerHTML = `Нажимая на кнопку, Вы даете согласие на обработку персональных данных в соответствии <a href="./privacy.docx">с&nbsp;политикой конфиденциальности`
            })
        } catch (e) {
            console.log("Нет текста из форм")
        }
    }

    addPoliticaText()

//  Нормальный скрол до элементов
    function fixScrollToAnchor() {
        // Получите высоту вашей шапки
        let headerHeight = document.querySelector('.menu').offsetHeight;

        // Обработчик события для якорных ссылок
        document.querySelectorAll('a[href^="#"]').forEach(anchor => {
            if (!anchor.closest(".akcii_elemts_js") && !anchor.closest(".price_table_content_js")){
                anchor.addEventListener('click', (e) => {
                    e.preventDefault();

                    // Получите ID элемента, к которому нужно прокрутить страницу
                    ...
                    if (targetElement) {
                        // Вычислите позицию для прокрутки с учетом высоты шапки
                        const scrollToPosition = targetElement.offsetTop - headerHeight;

                        // Используйте метод scrollTop для прокрутки страницы с плавной анимацией
                        window.scrollTo({
